...
public class CustomerServiceImpl extends ClientService implements CustomerService {
	...
	public List<Coupon> getCustomerCoupon(Category category) {
		return getCustomerCoupon().stream().filter(x -> x.getCategory().compareTo(category) == 0)
				.collect(Collectors.toList());
	}

	@Override
	public List<Coupon> getCustomerCoupon(double maxPrice) {
		return getCustomerCoupon().stream().filter(x -> x.getPrice() <= maxPrice).collect(Collectors.toList());
	}
