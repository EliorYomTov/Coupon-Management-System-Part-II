...
public class CompanyServiceImpl extends ClientService implements CompanyService {
	...
	public void addCoupon(Coupon coupon) throws CouponSystemException {
		List<Coupon> res = couponRepository.findCouponByTitle(coupon.getTitle(), this.companyId);
		if (res.size() > 0) {
	...
	public List<Coupon> getCompanyCoupons(Category category) {
		...
		couponRepository.findAllCouponsByCompanyId(companyId).stream().filter(x -> x.getCategory() == category)
	...
	public List<Coupon> getCompanyCoupons(Double maxPrice) {
		List<Coupon> result = new ArrayList<>();
		couponRepository.findAllCouponsByCompanyId(companyId).stream().filter(x -> x.getPrice() <= maxPrice)
				.forEach(x -> result.add(x));
		return result;
	}
